...
router.put('/',requireAuth,async(req,res,next)=>{try{
 ...
 const validationErrors=validateWorkspacePayload(incoming,req.user.role);if(validationErrors.length)return res.status(422).json({error:'INVALID_WORKSPACE',message:'Некорректная структура рабочего пространства',details:validationErrors});
 const result=await transaction(async client=>{
  ...
  if(!merged.changedSections.length){const users=await loadUsers(client,req.user.organizationId),revision=Number(current.revision);return{status:200,revision,schemaVersion:current.schema_version,data:publicWorkspace(current.data,req.user,users.rows,revision),changedSections:[]}}
  ...
  return{status:200,revision:nextRevision,schemaVersion,data:publicWorkspace(merged.data,req.user,users.rows,nextRevision),changedSections:merged.changedSections};
 });
