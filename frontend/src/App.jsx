...
export default function App(){
 ...
 const[section,setSection]=useState(isDirector?'director':'manager'),[activeId,setActiveId]=useState('w1'),[selectedId,setSelectedId]=useState(null),[query,setQuery]=useState(''),[showNew,setShowNew]=useState(false),[showTenderCapture,setShowTenderCapture]=useState(false),[notice,setNotice]=useState('');
 ...
 const tenderSaved=opportunity=>{setShowTenderCapture(false);setSection('opportunities');setSelectedId(null);setNotice(opportunity?.captureIncomplete?'Тендер сохранён. Дополните карточку перед оценкой.':'Тендер добавлен в очередь «Новые»')};
 ...
 return <div className={`v2-shell ${showContext?'with-context':''}`}>
  ...
  {showTenderCapture&&!isDirector&&<QuickTenderCapture data={data} setData={setData} currentUser={currentUser} onClose={()=>setShowTenderCapture(false)} onSaved={tenderSaved}/>} 
