...
export default function App(){
 ...
 useEffect(()=>{if(isDirector)return;const params=new URLSearchParams(window.location.search);if(params.get('capture')!=='1')return;const text=[params.get('title'),params.get('text'),params.get('url')].filter(Boolean).join('\n');setCaptureInput(text);setShowTenderCapture(true);params.delete('capture');params.delete('title');params.delete('text');params.delete('url');const next=`${window.location.pathname}${params.toString()?`?${params}`:''}${window.location.hash}`;window.history.replaceState({},'',next)},[isDirector]);
 ...
 const tenderSaved=opportunity=>{setShowTenderCapture(false);setCaptureInput('');setSection('opportunities');setSelectedId(null);setNotice(opportunity?.captureIncomplete?'Тендер сохранён. Дополните карточку перед оценкой.':'Тендер добавлен в очередь «Новые»')};
 ...
 return <div className={`v2-shell ${showContext?'with-context':''}`}>
  ...
  <div className="v2-content">
   ...
   {visibleSection==='opportunities'&&<OpportunityEngine data={data} setData={setData} currentUser={currentUser} onOpenWork={openWork} onAddTender={openTenderCapture}/>} 
  ...
  {showTenderCapture&&!isDirector&&<QuickTenderCapture data={data} setData={setData} currentUser={currentUser} initialCapture={captureInput} onClose={()=>{setShowTenderCapture(false);setCaptureInput('')}} onSaved={tenderSaved}/>} 
