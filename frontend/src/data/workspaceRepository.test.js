...
describe('workspace repository backups',()=>{
 ...
 it('exports and imports a validated workspace',()=>{const original=createDemoWorkspace();original.works[0].customer='Проверочный заказчик';const text=repository.exportText(original),restored=repository.importText(text);expect(restored.works[0].customer).toBe('Проверочный заказчик');expect(restored.schemaVersion).toBe(5)});
