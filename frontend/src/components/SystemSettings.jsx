...
import{AlertTriangle,Database,Download,FileUp,HardDriveDownload,RefreshCcw,Save,Settings2,ShieldCheck}from'lucide-react';
import'./system.css';

export function SystemSettings({data,setData,storageError,exportBackup,importBackup,restoreBackup,createSnapshot,reset}){
 ...
 const patchSettings=change=>setData(current=>({...current,settings:{...current.settings,...change}}));
 return<main className="system-page">
  ...
  <section className="system-finance"><div className="mini-title"><Settings2/>Финансовые параметры по умолчанию</div><div className="system-form"><label>Валюта<select value={data.settings?.currency||'RUB'} onChange={event=>patchSettings({currency:event.target.value})}><option value="RUB">RUB — ₽</option><option value="USD">USD — $</option><option value="EUR">EUR — €</option><option value="ILS">ILS — ₪</option></select></label><label>НДС, %<input type="number" min="0" max="100" value={data.settings?.vatRate??20} onChange={event=>patchSettings({vatRate:Math.max(0,Number(event.target.value||0))})}/></label><label>Целевая маржа, %<input type="number" min="0" max="100" value={data.settings?.targetMargin??15} onChange={event=>patchSettings({targetMargin:Math.max(0,Number(event.target.value||0))})}/></label></div><p>Новые позиции получают эти значения автоматически. Для отдельной позиции НДС, логистика и прочие расходы меняются в ее карточке.</p></section>
