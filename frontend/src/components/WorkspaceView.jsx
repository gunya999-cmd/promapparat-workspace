...
export function WorkspaceView({work,data,setData,selectedId,setSelectedId}){
 ...
 return <main className="workspace">
  ...
  <div className="table-wrap"><table><thead><tr><th>№</th><th>Позиция</th><th>Кол-во</th><th>Предложения</th><th>Выбрано</th><th>Экономика</th><th>Срок</th><th>Следующий шаг</th><th>Партии</th></tr></thead><tbody>{work.positions.map(position=><tr key={position.id} className={`${position.warnings.length?'risk-row':''} ${selectedId===position.id?'selected-row':''}`} onClick={()=>setSelectedId(position.id)}>
   <td>{position.rowNo}</td>
   <td><b>{position.name}</b><small>{position.group}</small></td>
   <td>{position.qty} {position.unit}</td>
   <td><span className="count-pill">{position.offers.length}</span>{position.offers.length>1&&<small>есть выбор</small>}</td>
   <td>{position.selected?<><b>{position.selected.supplier?.name||'Поставщик'}</b><small>{money(position.selected.price)}</small></>:<span className="bad">не выбрано</span>}</td>
   <td><b>{money(position.purchase)} → {money(position.sale)}</b><small className={position.margin!==null&&position.margin<15?'bad':'good'}>{pct(position.margin)} · {money(position.profit)}</small></td>
   <td>{position.selected?<>{position.selected.productionDays||0}+{position.selected.deliveryDays||0} дн.<small>{position.selected.shipmentPlace}</small></>:'—'}</td>
   <td><span className={`step ${position.nextStep==='Готово к КП'?'ok-step':''}`}>{position.nextStep}</span></td>
   <td>{position.batches?.length||0}<small>{position.shippedQty}/{position.qty} отгр.</small></td>
  </tr>)}</tbody></table></div>
 </main>
}
