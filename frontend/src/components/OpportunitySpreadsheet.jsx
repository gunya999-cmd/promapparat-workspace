...
export function OpportunitySpreadsheet({data,setData,currentUser,selectedId,setSelectedId,onOpenWork,onAccept,onReject}){
 ...
 return <section className="opp-sheet-shell">
  ...
  <div className="opp-sheet-toolbar">
   ...
   <select value={status} onChange={event=>setStatus(event.target.value)}><option>Все</option>{OPPORTUNITY_STATUSES.map(item=><option key={item}>{item}</option>)}</select>
   <select value={platform} onChange={event=>setPlatform(event.target.value)}><option>Все</option>{(data.platforms||[]).map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select>
   <select value={owner} onChange={event=>setOwner(event.target.value)}><option>Все</option>{owners.map(item=><option key={item}>{item}</option>)}</select>
   <Filter/>
  </div>
  ...
  <div className="opp-sheet-wrap">
   <table className="opp-sheet">
    ...
    <tbody>{list.map(item=><tr key={item.id} className={selectedId===item.id?'active-row':''} onClick={()=>setSelectedId(item.id)}>
     ...
     <td><input value={item.customer} onChange={event=>update(item.id,{customer:event.target.value})}/></td>
     <td><textarea value={item.title} onChange={event=>update(item.id,{title:event.target.value})}/></td>
     <td><select value={item.platformId||''} onChange={event=>update(item.id,{platformId:event.target.value})}>{(data.platforms||[]).map(option=><option value={option.id} key={option.id}>{option.name}</option>)}</select></td>
     <td><input value={item.externalId||''} onChange={event=>update(item.id,{externalId:event.target.value})}/></td>
     <td><div className="money-cell"><input type="number" min="0" value={item.estimatedAmount||''} onChange={event=>update(item.id,{estimatedAmount:event.target.value})}/><small>{compact(item.estimatedAmount)} · {money(item.estimatedAmount,data.settings?.currency||'RUB')}</small></div></td>
     <td><input type="date" value={item.deadline||''} onChange={event=>update(item.id,{deadline:event.target.value})}/></td>
     <td><select value={item.owner||''} onChange={event=>update(item.id,{owner:event.target.value})}><option value="">Не назначен</option>{owners.map(option=><option key={option}>{option}</option>)}</select></td>
