...
export function OpportunitySpreadsheet({data,setData,currentUser,selectedId,setSelectedId,onOpenWork,readOnly=false}){
 ...
 const allSelected=!readOnly&&list.length>0&&list.every(item=>selectedIds.includes(item.id));
 ...
 return <section className="opp-sheet-shell">
  ...
  <div className="opp-sheet-wrap"><table className="opp-sheet"><thead><tr><th className="check-cell">{!readOnly&&<button onClick={toggleAll}>{allSelected?<CheckSquare/>:<Square/>}</button>}</th><th>Заказчик</th><th>Предмет закупки</th><th>Площадка</th><th>№ закупки</th><th>Сумма</th><th>Дедлайн</th><th>Менеджер</th><th>Статус</th><th></th></tr></thead><tbody>{list.map(item=><tr key={`${item.id}-${item.updatedAt||''}`} className={selectedId===item.id?'active-row':''} onClick={()=>setSelectedId(item.id)}>
   ...
   <td>{readOnly?<span>{platformMap.get(item.platformId)?.name||'—'}</span>:<select value={item.platformId||''} onChange={event=>update(item.id,{platformId:event.target.value})} onClick={event=>event.stopPropagation()}>{(data.platforms||[]).map(option=><option value={option.id} key={option.id}>{option.name}</option>)}</select>}</td>
