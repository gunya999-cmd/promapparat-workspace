...
export function QuickTenderCapture({data,setData,currentUser,onClose,onSaved}){
 const fileRef=useRef(null),[form,setForm]=useState(blank),[error,setError]=useState(''),[busy,setBusy]=useState(false);
 const platforms=data.platforms||[],parsed=useMemo(()=>analyzeTenderPaste(form.pastedText||form.sourceUrl,platforms),[form.pastedText,form.sourceUrl,platforms]);
 const detectedPlatform=platforms.find(item=>item.id===(form.platformId||parsed.platformId));
 const applyPaste=value=>{const next=analyzeTenderPaste(value,platforms);setForm(current=>({...current,pastedText:value,sourceUrl:next.sourceUrl||current.sourceUrl,platformId:current.platformId||next.platformId,externalId:current.externalId||next.externalId,title:current.title||next.title,customer:current.customer||next.customer}));setError('')};
 ...
 const addFiles=files=>setForm(current=>({...current,attachments:[...current.attachments,...Array.from(files||[]).map(file=>({name:file.name,size:file.size,type:file.type}))]}));
 const removeFile=index=>setForm(current=>({...current,attachments:current.attachments.filter((_,itemIndex)=>itemIndex!==index)}));
 const save=()=>{if(!form.pastedText.trim()&&!form.sourceUrl.trim()&&!form.customer.trim()&&!form.title.trim()){setError('Вставьте ссылку или укажите хотя бы заказчика либо предмет закупки.');return}setBusy(true);try{let created;setData(current=>{const draft=buildManualTenderDraft(form,current.platforms||[],currentUser),result=createOpportunity(current,draft,currentUser);created=result.opportunity;return result.state});onSaved?.(created)}catch(exception){setError(exception?.message||'Не удалось добавить тендер');setBusy(false)}};
 return <div className="capture-backdrop" onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}>
  <section className="tender-capture" role="dialog" aria-modal="true" aria-labelledby="capture-title">
   ...
   <div className="capture-grid">
    ...
    <label><span>Площадка</span><select value={form.platformId||parsed.platformId||''} onChange={event=>setForm({...form,platformId:event.target.value})}><option value="">Другая / не определена</option>{platforms.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
    <label><span>Номер процедуры</span><input value={form.externalId||parsed.externalId||''} onChange={event=>setForm({...form,externalId:event.target.value})} placeholder="Определяется из ссылки"/></label>
   ...
   {form.attachments.length>0&&<div className="capture-file-list">{form.attachments.map((file,index)=><div key={`${file.name}-${index}`}><FilePlus2/><span>{file.name}</span><button onClick={()=>removeFile(index)}><X/></button></div>)}</div>}
