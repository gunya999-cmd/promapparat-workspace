...
import{uploadServerFiles}from'../api/client.js';
...
export function QuickTenderCapture({data,setData,currentUser,onClose,onSaved,initialCapture=''}){
 const fileRef=useRef(null),[form,setForm]=useState(blank),[error,setError]=useState(''),[busy,setBusy]=useState(false),[dragging,setDragging]=useState(false),platforms=data.platforms||[],serverManaged=data.meta?.serverManaged===true;
 ...
 const save=async()=>{if(busy)return;if(!form.pastedText.trim()&&!form.sourceUrl.trim()&&!form.customer.trim()&&!form.title.trim()){setError('Вставьте ссылку или укажите хотя бы заказчика либо предмет закупки.');return}setBusy(true);setError('');try{const opportunityId=uid(),baseDraft={...buildManualTenderDraft({...form,attachments:[]},data.platforms||[],currentUser),id:opportunityId};createOpportunity(data,baseDraft,currentUser);let attachments=[];if(serverManaged&&form.attachments.length){const uploaded=await uploadServerFiles(form.attachments.map(item=>item.file).filter(Boolean),'opportunity',opportunityId);attachments=(uploaded.files||[]).map(item=>({name:item.original_name,size:Number(item.size_bytes||0),type:item.mime_type||'',addedAt:item.created_at,serverFileId:item.id,storedOnServer:true,storedLocally:false}))}else attachments=await persistAttachmentEntries(form.attachments);const result=createOpportunity(data,{...baseDraft,attachments},currentUser);setData(result.state);onSaved?.(result.opportunity)}catch(exception){setError(exception?.message||'Не удалось добавить тендер');setBusy(false)}};
 ...
 return <div className={`capture-backdrop ${dragging?'is-dragging':''}`} onDragEnter={event=>{event.preventDefault();setDragging(true)}} onDragOver={event=>event.preventDefault()} onDragLeave={event=>{if(event.target===event.currentTarget)setDragging(false)}} onDrop={event=>{event.preventDefault();addFiles(event.dataTransfer.files)}} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}>
  <section className="tender-capture" role="dialog" aria-modal="true" aria-labelledby="capture-title">
   ...
   <section className={`capture-files ${dragging?'dragging':''}`} onClick={()=>fileRef.current?.click()}><div><Paperclip/><div><b>Перетащите документы из Проводника</b><span>{serverManaged?'Файлы загрузятся в общее серверное хранилище и будут доступны коллегам.':'Файлы сохранятся локально на этом компьютере.'}</span></div></div><input ref={fileRef} hidden type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.png,.jpg,.jpeg" onChange={event=>addFiles(event.target.files)}/><button type="button" onClick={event=>{event.stopPropagation();fileRef.current?.click()}}><UploadCloud/>Выбрать файлы</button></section>
   ...
   <footer className="capture-footer"><div><b>Быстрое сохранение</b><span>{serverManaged?'Тендер и файлы попадут в общую базу.':'Файлы останутся на этом Windows-компьютере.'} Нажмите Ctrl+Enter.</span></div><button onClick={onClose}>Отмена</button><button className="primary" disabled={busy} onClick={save}><Plus/>{busy?'Сохранение…':'Сохранить тендер'}<kbd>Ctrl+Enter</kbd></button></footer>
  </section>
 </div>;
}
