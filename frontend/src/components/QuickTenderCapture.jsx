...
export function QuickTenderCapture({data,setData,currentUser,onClose,onSaved,initialCapture=''}){
 ...
 const save=()=>{if(busy)return;if(!form.pastedText.trim()&&!form.sourceUrl.trim()&&!form.customer.trim()&&!form.title.trim()){setError('Вставьте ссылку или укажите хотя бы заказчика либо предмет закупки.');return}setBusy(true);try{let created;setData(current=>{const draft=buildManualTenderDraft(form,current.platforms||[],currentUser),result=createOpportunity(current,draft,currentUser);created=result.opportunity;return result.state});onSaved?.(created)}catch(exception){setError(exception?.message||'Не удалось добавить тендер');setBusy(false)}};
 ...
 return <div className={`capture-backdrop ${dragging?'is-dragging':''}`} onDragEnter={event=>{event.preventDefault();setDragging(true)}} onDragOver={event=>event.preventDefault()} onDragLeave={event=>{if(event.target===event.currentTarget)setDragging(false)}} onDrop={event=>{event.preventDefault();addFiles(event.dataTransfer.files)}} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}>
  <section className="tender-capture" role="dialog" aria-modal="true" aria-labelledby="capture-title">
   ...
   <section className={`capture-files ${dragging?'dragging':''}`} onClick={()=>fileRef.current?.click()}><div><Paperclip/><div><b>Перетащите документы из Проводника</b><span>Или нажмите сюда, чтобы выбрать PDF, Excel, Word, архивы и изображения.</span></div></div><input ref={fileRef} hidden type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.png,.jpg,.jpeg" onChange={event=>addFiles(event.target.files)}/><button type="button" onClick={event=>{event.stopPropagation();fileRef.current?.click()}}><UploadCloud/>Выбрать файлы</button></section>
   ...
   <footer className="capture-footer"><div><b>Быстрое сохранение</b><span>Нажмите Ctrl+Enter. Карточку можно дополнить позже.</span></div><button onClick={onClose}>Отмена</button><button className="primary" disabled={busy} onClick={save}><Plus/>{busy?'Сохранение…':'Сохранить тендер'}<kbd>Ctrl+Enter</kbd></button></footer>
  </section>
 </div>;
}