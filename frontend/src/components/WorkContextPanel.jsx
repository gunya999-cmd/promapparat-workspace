...
export function WorkContextPanel({work,data,onSelectPosition}){
 ...
 return <aside className="context-panel"><div className="context-head"><span>Контекст тендера</span><h2>{work.customer}</h2><p>{work.code} · {work.state}</p></div>
  <section className="context-action"><div><Target/><span>Следующее действие</span></div><h3>{next?.nextStep||work.nextAction}</h3>{next&&<p>Позиция №{next.rowNo} · {next.name}</p>}{next&&<button onClick={()=>onSelectPosition(next.id)}>Открыть позицию<ArrowRight/></button>}</section>
