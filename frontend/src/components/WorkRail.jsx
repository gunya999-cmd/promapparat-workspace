...
import{BriefcaseBusiness,Building2,LayoutDashboard,Plus,Search,Zap}from'lucide-react';
...
export function WorkRail({works,activeId,onSelect,onNew,query,setQuery,section,setSection}){
 return <aside className="rail">
  ...
  <div className="rail-nav"><button className={section==='dashboard'?'active':''} onClick={()=>setSection('dashboard')}><LayoutDashboard/>Центр управления</button><button className={section==='works'?'active':''} onClick={()=>setSection('works')}><BriefcaseBusiness/>Работы</button><button className={section==='suppliers'?'active':''} onClick={()=>setSection('suppliers')}><Building2/>Поставщики</button></div>
