...
import{BriefcaseBusiness,Building2,Calculator,Compass,Crown,LayoutDashboard,Plus,Search,Settings,ShieldCheck,Zap}from'lucide-react';
...
export function WorkRail({works,activeId,onSelect,onNew,query,setQuery,section,setSection,currentUser,isAdmin}){
 const isHead=currentUser?.role==='head',roleLabel=isAdmin?'Директор / администратор':isHead?'Руководитель отдела':'Тендерный менеджер';
 return <aside className="rail">
  <div className="brand"><div className="logo">PA</div><div><b>PromApparat</b><span>Opportunity OS</span></div></div>
  <div className="rail-nav"><button className={section==='opportunities'?'active':''} onClick={()=>setSection('opportunities')}><Compass/>Возможности</button><button className={section==='dashboard'?'active':''} onClick={()=>setSection('dashboard')}><LayoutDashboard/>{isHead||isAdmin?'Центр управления':'Рабочий день'}</button><button className={section==='works'?'active':''} onClick={()=>setSection('works')}><BriefcaseBusiness/>{isHead||isAdmin?'Проекты в работе':'Мои проекты'}</button><button className={section==='suppliers'?'active':''} onClick={()=>setSection('suppliers')}><Building2/>Поставщики</button>{isAdmin&&<><button className={section==='director'?'active director-nav':''} onClick={()=>setSection('director')}><Crown/>Бизнес директора <ShieldCheck size={13}/></button><button className={section==='formulas'?'active':''} onClick={()=>setSection('formulas')}><Calculator/>Формулы <ShieldCheck size={13}/></button><button className={section==='system'?'active':''} onClick={()=>setSection('system')}><Settings/>Администрирование <ShieldCheck size={13}/></button></>}</div>
  ...
  {section==='opportunities'&&<div className="rail-section-note"><Compass/><b>Охота за тендерами</b><span>Площадки, найденные закупки, оценка и перевод в проект.</span></div>}
  ...
  {section==='director'&&isAdmin&&<div className="rail-section-note"><Crown/><b>Только директору</b><span>Деньги, прогноз, эффективность, риски и вся цепочка до первичных данных.</span></div>}
