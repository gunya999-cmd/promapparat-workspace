...
const deadlineLabel=value=>{const days=daysLeft(value);if(days<0)return`проср. ${Math.abs(days)} дн.`;if(days===0)return'сегодня';if(days===1)return'завтра';return`${days} дн.`};
...
export function WorkRail({works,activeId,onSelect,onNew,query,setQuery,section,setSection,currentUser,isAdmin}){
 return <aside className="rail">
  ...
  <div className="rail-nav"><button className={section==='dashboard'?'active':''} onClick={()=>setSection('dashboard')}><LayoutDashboard/>Центр управления</button><button className={section==='works'?'active':''} onClick={()=>setSection('works')}><BriefcaseBusiness/>Работы</button><button className={section==='suppliers'?'active':''} onClick={()=>setSection('suppliers')}><Building2/>Поставщики</button>{isAdmin&&<><button className={section==='formulas'?'active':''} onClick={()=>setSection('formulas')}><Calculator/>Формулы <ShieldCheck size={13}/></button><button className={section==='system'?'active':''} onClick={()=>setSection('system')}><Settings/>Система <ShieldCheck size={13}/></button></>}</div>
  ...
  {section==='system'&&isAdmin&&<div className="rail-section-note"><Settings/><b>Безопасность данных</b><span>Backup, восстановление и миграции.</span></div>}
  <div className="rail-section-note"><ShieldCheck/><b>{currentUser?.name||'Пользователь'}</b><span>{isAdmin?'Администратор':currentUser?.role==='head'?'Руководитель отдела':'Менеджер'}</span></div>
 </aside>;
}
