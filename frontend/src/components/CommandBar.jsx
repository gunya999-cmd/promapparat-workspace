...
const sectionTitle=section=>({manager:'Рабочий стол менеджера',opportunities:'Поиск тендеров',dashboard:'Рабочий день',works:'Карточка сделки',suppliers:'Поставщики',director:'Бизнес-аналитика директора',finance:'Финансовый центр директора',formulas:'Конструктор формул',system:'Система'}[section]||'PromApparat Workspace');
...
export function CommandBar({works,currentUser,onOpenWork,onNew,section}){
 ...
 return <header className="command-bar">
  ...
  <div className="command-actions"><button className="command-new" onClick={onNew}><Plus/>Новая сделка</button><button className="command-icon" title="Уведомления"><Bell/></button><div className="command-user"><UserRound/><div><b>{currentUser?.name||'Пользователь'}</b><span>{currentUser?.role==='director'?'Директор':'Менеджер'}</span></div></div></div>
 </header>;
}
