...
export function CommandBar({works,currentUser,onOpenWork,onAddTender,onLogout,serverManaged=false,syncState='',onReload,section}){
 ...
 return <header className="command-bar">
  ...
  <div className="command-actions">{serverManaged?<><div className={`server-sync ${syncState}`} title={syncLabel(syncState)}><Cloud/><span>{syncLabel(syncState)}</span>{syncState==='conflict'&&<button onClick={onReload} title="Загрузить актуальные данные"><RefreshCw/></button>}</div><div className="server-user"><span><b>{currentUser?.name}</b><small>{isDirector?'Директор':'Менеджер'}</small></span>{isDirector?<ShieldCheck/>:<UserRound/>}</div><button className="command-icon" onClick={onLogout} title="Выйти"><LogOut/></button></>:<div className="server-user"><span><b>{currentUser?.name||'Пользователь'}</b><small>{isDirector?'Директор':'Менеджер'}</small></span>{isDirector?<ShieldCheck/>:<UserRound/>}</div>}{!isDirector&&<button className="command-new" onClick={onAddTender}><Clipboard/>Добавить тендер</button>}<button className="command-icon" title="Уведомления"><Bell/></button></div>
 </header>;
}
