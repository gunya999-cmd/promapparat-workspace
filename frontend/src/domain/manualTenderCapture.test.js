...
describe('manual tender capture',()=>{
 ...
 it('reads labelled customer, amount and deadline without guessing from arbitrary text',()=>{const result=analyzeTenderPaste('Предмет закупки: Регулирующие клапаны\nЗаказчик: ООО Север\nНМЦК: 14 500 000,00 руб.\nСрок подачи: 31.08.2026\nДополнительный выделенный текст',platforms);expect(result.title).toBe('Регулирующие клапаны');expect(result.customer).toBe('ООО Север');expect(result.estimatedAmount).toBe(14500000);expect(result.deadline).toBe('2026-08-31')});
