...
const paymentWorkflow=(state,workId)=>{
 const work=state.works.find(item=>item.id===workId),invoices=(state.invoices||[]).filter(item=>item.workId===workId),billed=invoices.reduce((sum,item)=>sum+Number(item.amount||0),0),paid=invoices.reduce((sum,item)=>sum+invoicePaid(item),0),saleTotal=state.positions.filter(position=>position.workId===workId).reduce((sum,position)=>sum+Number(position.salePrice||0)*Number(position.qty||0),0),debt=Math.max(0,billed-paid),remaining=Math.max(0,saleTotal-billed),overdue=invoices.some(item=>invoiceStatus(item)==='Просрочен'),fullyPaid=saleTotal>0&&paid>=saleTotal&&debt<=0,activeState=work?.state==='Закрыто успешно'?'Закрыто успешно':'Отгрузка';
 ...
 if(overdue)return{state:activeState,paymentStatus:'Просрочено',nextAction:'Получить просроченную оплату',nextActionDate:todayPlus(1),paymentSummary:{billed,paid,debt,remaining}};
 if(paid>0)return{state:activeState,paymentStatus:'Частичная оплата',nextAction:remaining>0?'Выставить оставшийся счёт':'Получить остаток оплаты',nextActionDate:invoices.map(item=>item.dueDate).filter(Boolean).sort()[0]||todayPlus(1),paymentSummary:{billed,paid,debt,remaining}};
 if(invoices.length)return{state:activeState,paymentStatus:'Счет выставлен',expectedPaymentDate:invoices.map(item=>item.dueDate).filter(Boolean).sort()[0]||'',nextAction:'Контролировать оплату счёта',nextActionDate:invoices.map(item=>item.dueDate).filter(Boolean).sort()[0]||todayPlus(1),paymentSummary:{billed,paid,debt,remaining}};
