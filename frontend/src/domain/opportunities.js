...
export function acceptOpportunity(state,id,actor){
 ...
 const works=created.state.works.map(item=>item.id===work.id?work:item),sourceDocuments=attachments.map(attachment=>({id:uid(),workId:work.id,positionId:null,type:sourceDocumentType(attachment.name),name:attachment.name,size:Number(attachment.size||0),mime:attachment.type||'',uploadedAt:attachment.addedAt||now(),required:['ТЗ','Спецификация'].includes(sourceDocumentType(attachment.name)),source:'tender-capture',storageKey:attachment.storageKey||'',storedLocally:attachment.storedLocally!==false,serverFileId:attachment.serverFileId||'',storedOnServer:attachment.storedOnServer===true})),next={...current,status:'Взята в работу',workId:work.id,decidedAt:now(),decidedBy:actor?.id||null};
